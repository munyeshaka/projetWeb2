...
<?php
	...
	$affichagBuySac= $bdd->query("SELECT* from prodsac where idProdSac='".$_GET["buy"]."'");

	$dataProdSac= $affichagBuySac->fetch();
	 
?>
		<div id="produit">
			<br>
		<tr>
      		<td><a id="a" href="BoutiquePay.php"><?php echo '<img style="height:200px; width:200px;" src="media/'.$dataProdSac["photo"].'">';?></a></td>
      	</tr>
      </div>
...
{
	$demande= $bdd->query('SELECT * from prodsac where idProdSac ="'.$_GET["buy"].'"');
	$data= $demande->fetch();
	$ret= $data["stock"];
	...
	{
		$rest=$ret-$quantite;
		$dimune= $bdd->query("UPDATE prodsac set stock='".$rest."' where idProdSac='".$data["idProdSac"]."'");
	}
}
?>
