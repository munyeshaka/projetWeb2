...
<?php
	...
	$affichagBuyPro= $bdd->query("SELECT* from produit where idProduit='".$_GET["buy"]."'");

	$dataProd= $affichagBuyPro->fetch();
...
<div id="produit">
	<br>
	<tr>
       	<td><a id="a" href="BoutiquePay.php"><?php echo '<img style="height:200px; width:200px;" src="media/'.$dataProd['photo'].'">';?></a></td>
...
{
	$demande= $bdd->query('SELECT * from produit where idProduit ="'.$_GET["buy"].'"');
	$data= $demande->fetch();
	$ret= $data["stock"];
	...
	{
		$rest=$ret-$quantite;
		$dimune= $bdd->query("UPDATE produit set stock='".$rest."' where idProduit='".$data["idProduit"]."'");
	}
}
?>
