...
<h1>
	...
	<p><a id="a" href="BoutiqueAjouterProdSac.php">Ajouter Sac</a></p>
</h1>
...
if(isset($_POST["Ajouter"]))
{
	...
	$recupAlldeClient='SELECT* from produit where title=:titleStocks, photo=:photoStock, prix=:prixStocks, stock=:stockStocks';
...
{
	$EnregistreUtilisateur= $bdd->prepare('INSERT INTO produit(title, photo, prix, stock)VALUES(?,?,?,?)'); //les ??? sont les contenus du titleClient,email,.. 
	$EnregistreUtilisateur->execute(array($recupTitle, $recupPhoto, $recupPrix, $recupStock
	));

header("Location: BoutiqueAjouterProduit.php");
}
}
