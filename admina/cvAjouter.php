...
<form METHOD="POST">
	<table>
		...
		<td><input type="submit" name="EnregistreCV" value="Enregistrer CV"></td>
    
	</table>
</form>

<?php
if(isset($_POST["EnregistreCV"]))
{
	$recupanneeAutre= $_POST["anneeAutre"];
	$recupactionCompetence= $_POST["actionCompetence"];

	$recupAllanneeAutreDB='SELECT* from cv where anneeAutre=:anneeAutreStocks && actionCompetence=:actionCompetenceStocks';
...
else
{
	...
	$insertionanneeAutre= $bdd->prepare('INSERT INTO cv(anneeAutre, actionCompetence)VALUES(:anneeAutre,:actionCompetence)');
